...
class AppStore extends Component {
  ...
  searchApp = event => {
    this.setState({inputIs: event.target.value})
  }

  getActiveTabs = () => {
    const {tabIdIs} = this.state
    const appCategotyList = appsList.filter(
      appItem => appItem.category === tabIdIs,
    )
    return appCategotyList
  }

  activeTabare = appCategotyList => {
    const {inputIs} = this.state
    const searchAppList = appCategotyList.filter(appItem =>
      appItem.appName.toLocaleLowerCase().includes(inputIs.toLocaleLowerCase()),
    )
    return searchAppList
  }

  render() {
    const {tabIdIs, inputIs} = this.state
    const appCategotyList = this.getActiveTabs()
    const filteredApps = this.activeTabare(appCategotyList)
        ...
        <ul className="tabsList-container">
          {tabsList.map(tabItem => (
            <TabItem
              ...
              isChanged={tabItem.tabId === tabIdIs}
              value={inputIs}
            />
          ))}
        </ul>
